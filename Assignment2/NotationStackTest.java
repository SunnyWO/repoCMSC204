...
public class NotationStackTest {
	...
	public Double aDouble=1.1, bDouble=5.55, cDouble=123.0, dDouble=0.5, eDouble=0.0, fDouble=1.0;
	...
	public void setUp() throws Exception {
		...
		doubleS.push(bDouble);
		doubleS.push(cDouble);
	}
	...
	public void testPopStudent() {
		...
		try {
			assertEquals(cDouble, doubleS.pop());
			assertEquals(bDouble, doubleS.pop());
			assertEquals(aDouble, doubleS.pop());
			//Queue is empty, next statement should cause QueueUnderFlowException
			doubleS.pop();
			assertTrue("This should have caused an StackUnderflowException", false);
		}
	...
	public void testPushStudent() {
		...
		try {
			...
			assertEquals(true, doubleS.push(dDouble));
			assertEquals(4, doubleS.size());
			assertEquals(true, doubleS.push(eDouble));
			assertEquals(5, doubleS.size());
			//Queue is full, next statement should cause QueueOverFlowException
			doubleS.push(fDouble);
			assertTrue("This should have caused an StackOverflowException", false);
		}
		catch (StackOverflowException e){
			assertTrue("This should have caused an StackOverflowException", true);
	...
	public void testToStringStudent() throws StackOverflowException {
		//Use the doubleQ for student tests
		assertEquals("1.15.55123.0", doubleS.toString());
		doubleS.push(dDouble);
		assertEquals("1.15.55123.00.5", doubleS.toString());
		doubleS.push(eDouble);
		assertEquals("1.15.55123.00.50.0", doubleS.toString());
	}
